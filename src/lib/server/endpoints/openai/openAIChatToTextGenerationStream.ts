...
export async function* openAIChatToTextGenerationStream(
	...
	getRouterMetadata?: () => { route?: string; model?: string; provider?: InferenceProvider }
) {
	let generatedText = "";
	let tokenId = 0;
		...
		if (!metadataYielded && retyped["x-router-metadata"]) {
			...
			yield {
				...
				routerMetadata: {
					...
					provider: metadata.provider ? (metadata.provider as InferenceProvider) : undefined,
				},
		...
		if (
			...
			yield {
				...
				routerMetadata: routerMetadata ? {
					...
					provider: routerMetadata.provider ? (routerMetadata.provider as InferenceProvider) : undefined,
				} : undefined,
			} as TextGenerationStreamOutput & {
				routerMetadata: { route?: string; model?: string; provider?: InferenceProvider };
			};
